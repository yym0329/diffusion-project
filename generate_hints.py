...
def get_envmap(viewpoint_id: str, light_condition_id: str) -> str:
	if viewpoint_id not in ["NA3", "NE7", "CB5", "CF8", "NA7", "CC7", "CA2", "NE1", "NC3", "CE2"]:
		raise ValueError(f"Invalid viewpoint_id: {viewpoint_id}")
	if light_condition_id not in ["001", "002", "003", "004", "005", "006", "007", "008", "009", "010", "011", "012", "013"]:
...
def main(args: Args):
	...
	for item in metadata:
		...
		output_folder = os.path.join(args.output_dir, os.path.basename(image_path).split(".")[0])
		if not os.path.exists(output_folder):
			os.makedirs(output_folder)
		
		print(f"Rendering radiance hints for {image_path} with viewpoint {viewpoint_id} and lighting condition {lighting_condition_id}")
		...
		render_hint_images(mesh, fov, env_map= env_map , output_folder=output_folder, use_gpu=args.use_gpu_for_rendering)
